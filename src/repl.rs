...
use roc_load_internal::module::MonomorphizedModule;
use roc_parse::ast::Expr;
use roc_repl_eval::{
    ReplApp, ReplAppMemory,
    eval::jit_to_ast,
    r#gen::{ReplOutput, format_answer},
};
use roc_repl_ui::{
    TIPS, format_output,
    repl_state::{ReplAction, ReplState},
};
use roc_reporting::report::{ANSI_STYLE_CODES, DEFAULT_PALETTE_HTML, HTML_STYLE_CODES};
use roc_target::Target;
use roc_types::pretty_print::{DebugPrint, name_and_print_var};
use std::cell::RefCell;
use std::mem::size_of;

use crate::wasm_runner::{WasmtimeApp, wasmtime_create_app};
...
impl<'a> ReplApp<'a> for WasmReplApp<'a> {
    ...
    where
        ...
        Self::Memory: 'a,
    {
        let app_final_memory_size: usize = self.wasmtime_app.wasmtime_run_app();

        let copied_bytes: &mut [u8] = self.allocate_buffer(app_final_memory_size);

        let app_result_addr = self
            .wasmtime_app
            .wasmtime_get_result_and_memory(copied_bytes);

        let result_bytes: &[u8] = &copied_bytes[app_result_addr..];
...
pub async fn entrypoint_from_wasmtime(src: String) -> String {
...
async fn eval_wasm<'a>(
    ...
    let inner = match wasmtime_create_app(&app_module_bytes).await {
        Ok(inner) => inner,
        Err(ex) => {
